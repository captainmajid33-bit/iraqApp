 ...
 * Night : 18:00 – 05:59
 *
 * tileUrl        = _all variant  (labels + embedded POI icons) — وضع "الكل"
 * tileUrlFocused = _nolabels variant (بدون أيقونات POI مضمّنة) — وضع الفئة / الملاحة
 */
import { useState, useEffect } from 'react';

export interface MapTheme {
  isDay:          boolean;
  /** Normal tiles — show embedded labels + POI icons */
  tileUrl:        string;
  /** Focused tiles — no embedded POI icons (category-filter / nav mode) */
  tileUrlFocused: string;
...
const NIGHT: MapTheme = {
  ...
  tileUrlFocused: 'https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png',
...
const DAY: MapTheme = {
  ...
  tileUrlFocused: 'https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png',
